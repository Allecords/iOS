...
final class ChatPartnerTableViewCell: UITableViewCell {
...
extension ChatPartnerTableViewCell {
	func setTextView() {
		messageBox.isScrollEnabled = false
		messageBox.isEditable = false
		messageBox.backgroundColor = .primaryLight
	...
	func setupViews() {
		...
		NSLayoutConstraint.activate([
			messageBox.leadingAnchor.constraint(equalTo: contentView.leadingAnchor, constant: 15),
		...
		// date constraints
		NSLayoutConstraint.activate([
			date.leadingAnchor.constraint(equalTo: messageBox.trailingAnchor, constant: 5),
